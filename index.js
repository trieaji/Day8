...
app.get('/edit/:dex', function(request,response){ //menampilkan data menggunakan hbs atau memanggil
    let indexx = request.params.dex;
    //console.log(index);

    let edit = dataBlog[indexx];
    //console.log(edit);

    response.render('edit', { edit, indexx });
}) 

app.post('/edit/:dit', function(request, response){
    let index = request.params.dit;
    let tub = request.body // menampung data yang diinputkan untuk ditampilkan di halaman web
    tub = {
        Tittle: tub.inputTitle,
        Content: tub.inputCont,
        Author: 'Mas Bambang',
        postAt: new Date(),
        //isLogin // memanggil "isLogin" supaya bisa melakukan update data atau menambahkan data
    }
